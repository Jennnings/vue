<template>
  <div>
    <a-button type="primary" @click="createReceipt">
      新建发票
    </a-button>
    <a-table
      :columns="columns"
    ...
    <a-modal
      ...
      :destroyOnClose="distoryThis"
      :maskClosable="false"
      @cancel="reloadReceipt"
    >
      <EditReceiptModalForContract
...
import CreateReceiptModalForContract from "./CreateReceiptModalForContract";
import ViewReceiptDetail from "./ViewReceiptDetail";
import EditReceiptModalForContract from "./EditReceiptModalForContract";
const columns = [
  {
...
export default {
  ...
  methods: {
    async getReceipts() {
      this.spinning = true;
      const tmp_data = await request.get("/receipt/getcontractreceipt", {
        params: {
